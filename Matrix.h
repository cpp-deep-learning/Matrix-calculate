...
Matrix Matrix::operator+(const Matrix& M) {
  if (M.rows != 0 && M.column != 0 && rows != 0 && column != 0 &&
      (M.rows) == (rows) && (M.column) == (column)) {
...
Double& Matrix::operator()(Integer x0, Integer y0) {
...
std::istream& operator>>(std::istream& is,Matrix& M) {
	for (Integer i = 0; i < M.rows * M.column; i++) {
    	std::cout << "please input the Matrix[" << i / M.column + 1 << "]"<< "[" << i - (i / M.column) * M.column + 1 << "]" << std::endl;
    	std::cin >> M.value[i];
	}
	return is;
}

Double* M_out(const Matrix& M) {
  if(M.value!=nullptr) {
    Double* data=new Double[M.rows*M.column];
    for(Integer i=0;i<M.rows*M.column;i++) data[i]=M.value[i];
    return data;
  }
  else {
		...
		for(l=0;l<A.rows;l++) {
			for(k=0;k<B.rows;k++) {
				for(j=0;j<A.column;j++) {
					for(i=0;i<B.column;i++) result->value[(l*B.column+k)*A.column*B.column+j*B.column+i]=A.value[l*A.column+j]*B.value[k*B.column+i];
				}
			}
		}
		return *result;
